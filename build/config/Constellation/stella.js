...
class Stella {
  constructor(ip, hostname, credentials) {
    if (!validate.isIP(ip)) throw new Error("INVALID_ARGUMENT", "Invalid ip address");
    let creds = credentials.split(":");
    if (creds.length < 2 || _.isEmpty(creds[0]) || _.isEmpty(creds[1])) throw new Error("INVALID_ARGUMENT", "Invalid credentials");
