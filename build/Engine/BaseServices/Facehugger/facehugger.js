...
class Facehugger extends Abstract {
  ...
  init(params) {
    ...
    this.exposed_api = _.chain(this._db).functions().filter(name => {
  ...
  handleRequest(_ref) {
    let actname = _ref.action;
    let args = _ref.params;
    let mid = _ref.id;

    //        console.log("HANDLING REQUEST");
    return new Promise((resolve, reject) => {
      if (!actname || ! ~_.indexOf(this.exposed_api, actname)) return reject(new Error("MISSING_METHOD"));
