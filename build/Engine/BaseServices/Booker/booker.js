...
class Booker extends Abstract {
	...
	init(config) {
		...
		if (this.master) {
			...
			if (!this.slave || !this.slave_bucket || !this.master_bucket) return Promise.reject(new Error("SERVICE_ERROR", 'Specify all master-slave relations'));
			this.master_stella = mip = this.hosts.show(this.master);
			if (!mip) return Promise.reject(new Error("SERVICE_ERROR", 'Configure master constellation in hosts'));
	...
	patch(_ref) {
		...
		if (this.paused || this.master_stella && !this.master_stella.active) return Promise.reject(new Error("SERVICE_ERROR", "Service is paused"));
		...
		if (!mo) return Promise.reject(new Error("MISCONFIGURATION", "No such class in MetaTree"));
